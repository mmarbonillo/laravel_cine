...
@section('scripts')
    <script type="text/javascript">
        $().ready(function(){
            $("input.deleteButtonIndex").click(function(){
                var id = $(this).attr("id");
                var capa = $(this).parent();
                $.get("{{ route('movie.delete') }}", {movie: id}, function(resp){
                    capa.remove();
                });
            });
        });
...
@foreach($moviesList as $movie)
    <div class='movieIndex'>
        ...
        <input type='button' name='deletee' class='deleteButtonIndex' value='DELETE' id="{{$movie->id}}">
        @endauth
